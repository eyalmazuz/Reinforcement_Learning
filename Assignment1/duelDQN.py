...
class DQN(Model):
    ...
    def __init__(self, n_states, n_actions, n_layers):
        ...
        self.value_values = Dense(1, activation='linear')
    ...
    def call(self, x):
        ...
        state_value = self.value_values(x)

        q_values = state_value + (q_values - tf.reduce_mean(q_values, axis=1, keepdims=True))
...
def main():
    ...
    plt.savefig(f'./duel_dqn_rewards_{CUR_DATE}.png', )
    plt.clf()
    ...
    plt.savefig(f'./duel_dqn_steps_{CUR_DATE}.png')
